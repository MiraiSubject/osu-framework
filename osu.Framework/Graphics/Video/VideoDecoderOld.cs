...
namespace osu.Framework.Graphics.Video
{
    ...
    /// </summary>
    public unsafe class VideoDecoderOld : IDisposable
    {
        /// <summary>
        ...
        /// <summary>
        /// The current state of the <see cref="VideoDecoderOld"/>, as a bindable.
        /// </summary>
        ...
        private ObjectHandle<VideoDecoderOld> handle;
        ...
        public VideoDecoderOld(string filename, Scheduler scheduler, AVHWDeviceType hwDevice)
        ...
        /// <param name="scheduler">The <see cref="Scheduler"/> to use when scheduling tasks from the decoder thread.</param>
        /// <param name="hwDevice">The <see cref="AVHWDeviceType"/> to use when decoding the video.</param>
        public VideoDecoderOld(Stream videoStream, Scheduler scheduler, AVHWDeviceType hwDevice)
        {
            ffmpeg = CreateFuncs();
            ...
            if (!videoStream.CanRead)
                throw new InvalidOperationException($"The given stream does not support reading. A stream used for a {nameof(VideoDecoderOld)} must support reading.");
            ...
            handle = new ObjectHandle<VideoDecoderOld>(this, GCHandleType.Normal);
        }
        ...
        {
            var handle = new ObjectHandle<VideoDecoderOld>((IntPtr)opaque);
            if (!handle.GetTarget(out VideoDecoderOld decoder))
                return 0;
